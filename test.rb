...
class Wall
  ...
  def initialize(rock_image_path)
    @positions = []
    @rock_image_path = rock_image_path
    while true
	  ...
	  if (start_x).abs >= 6 || (start_y).abs >= 6 
	...
	i = 0
	  	while i < @positions.length
			position = @positions[i]
		    Image.new(@rock_image_path, x: position[0] * SQUARE_SIZE, y: position[1] * SQUARE_SIZE, z:ZOrder::GAME)	    
		    i += 1
		end
	end
...
class Game
	def initialize(snake, score_to_win, number_of_walls, level)
    @level = level
    @score = 0
    @finished = false
    @backgrounds_and_rocks = [
      ['./media/dungeon.jpg', './media/rock.jpg'],
      ['./media/dungeon-2.jpg', './media/rock-2.jpg'],
      ['./media/dungeon-3.jpg', './media/rock-3.jpg'],
      ['./media/dungeon-4.jpg', './media/rock-4.jpg']
    ]
    
    # Select the background and rock based on the current level
    @background_image_path, @rock_image_path = @backgrounds_and_rocks[(@level - 1) % @backgrounds_and_rocks.length]
    @background_image = Image.new(@background_image_path, z:ZOrder::BACKGROUND)
    @snake = snake
    @food_x = nil
    @food_y = nil
    @walls = []
    ...
    while i < @number_of_walls  # Repeat the following block until the counter reaches 20
      @walls << Wall.new(@rock_image_path)  # Create a new Wall object and add it to the array
      i += 1  
    end
    generate_food()
...
level = 1
snake = Snake.new()
game = Game.new(snake, score_to_win, number_of_walls, level)
...
on :key_down do |event|
    ...
    if game.win
      snake = Snake.new()
	    score_to_win = score_to_win + 2
	    number_of_walls = number_of_walls + 2
	    level  += 1
			game = Game.new(snake, score_to_win, number_of_walls, level)
			game_win.stop
			game_sound.stop
