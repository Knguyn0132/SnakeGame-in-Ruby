...
class Game
	def initialize(snake, number_of_walls)
		@score = 0
		@finished = false
	...
	def draw
		...
		if !finished?
			...
			Text.new("#{@score}/#{@score_to_win}", color: "#82171f", style:'bold', x:10, y:10, z:ZOrder::TEXT, size: 20)
		else 
			if win
...
number_of_walls = 10
snake = Snake.new()
game = Game.new(snake, number_of_walls)
